...
# 왕대코드 → 왕명. 벌크 id는 2nd_w?a; 가운데 글자 = 왕 순서(a=태조 … y=철종, 25대).
# 중복/수정 실록(선조수정·광해군중초정초·현종개수·숙종보궐·경종수정)은 별도 가운데
# 글자가 아니라 셋째 글자로 구분되며(w?a 가 아닌 w?b/w?c 등), 파서 정규식이 셋째
# 글자 'a'만 받으므로 자동으로 원본만 집계된다.
# 실데이터로 확정: waa=태조, wca=태종, wya=철종(마지막 파일) → 가운데 글자 = 왕 순서.
KING_CODES = {
    "a": "태조", "b": "정종", "c": "태종", "d": "세종", "e": "문종", "f": "단종",
    "g": "세조", "h": "예종", "i": "성종", "j": "연산군", "k": "중종", "l": "인종",
    "m": "명종", "n": "선조", "o": "광해군", "p": "인조", "q": "효종", "r": "현종",
    "s": "숙종", "t": "경종", "u": "영조", "v": "정조", "w": "순조", "x": "헌종",
    "y": "철종",
}
